"use client";

/**
 * Sticky left category nav for the Awards page (MoMorph mms_C_Menu list,
 * node 313:8459): 6 entries, one per award category. Clicking smooth-scrolls
 * to the matching `#slug` section and marks it active immediately; an
 * IntersectionObserver also tracks which section is in view as the user
 * scrolls manually (scroll-spy), so only one item is ever active at a time.
 * Active styling (gold text + underline + glow) mirrors the header's active
 * nav link, see `components/home/nav-links.tsx`.
 */
import { useCallback, useEffect, useState } from "react";

import { AWARDS_CATALOG } from "@/lib/awards/awards-catalog";

/** Small bullet icon rendered before every nav label (MM_MEDIA_Target, reused before every "title" row across this screen). */
function TargetIcon({ className }: { className?: string }) {
  return (
    <svg viewBox="0 0 24 24" fill="none" aria-hidden="true" className={className}>
      <path
        d="M12 2C10.6868 2 9.38642 2.25866 8.17317 2.7612C6.95991 3.26375 5.85752 4.00035 4.92893 4.92893C3.05357 6.8043 2 9.34784 2 12C2 14.6522 3.05357 17.1957 4.92893 19.0711C5.85752 19.9997 6.95991 20.7362 8.17317 21.2388C9.38642 21.7413 10.6868 22 12 22C14.6522 22 17.1957 20.9464 19.0711 19.0711C20.9464 17.1957 22 14.6522 22 12C22 10.84 21.79 9.69 21.39 8.61L19.79 10.21C19.93 10.8 20 11.4 20 12C20 14.1217 19.1571 16.1566 17.6569 17.6569C16.1566 19.1571 14.1217 20 12 20C9.87827 20 7.84344 19.1571 6.34315 17.6569C4.84285 16.1566 4 14.1217 4 12C4 9.87827 4.84285 7.84344 6.34315 6.34315C7.84344 4.84285 9.87827 4 12 4C12.6 4 13.2 4.07 13.79 4.21L15.4 2.6C14.31 2.21 13.16 2 12 2ZM19 2L15 6V7.5L12.45 10.05C12.3 10 12.15 10 12 10C11.4696 10 10.9609 10.2107 10.5858 10.5858C10.2107 10.9609 10 11.4696 10 12C10 12.5304 10.2107 13.0391 10.5858 13.4142C10.9609 13.7893 11.4696 14 12 14C12.5304 14 13.0391 13.7893 13.4142 13.4142C13.7893 13.0391 14 12.5304 14 12C14 11.85 14 11.7 13.95 11.55L16.5 9H18L22 5H19V2ZM12 6C10.4087 6 8.88258 6.63214 7.75736 7.75736C6.63214 8.88258 6 10.4087 6 12C6 13.5913 6.63214 15.1174 7.75736 16.2426C8.88258 17.3679 10.4087 18 12 18C13.5913 18 15.1174 17.3679 16.2426 16.2426C17.3679 15.1174 18 13.5913 18 12H16C16 13.0609 15.5786 14.0783 14.8284 14.8284C14.0783 15.5786 13.0609 16 12 16C10.9391 16 9.92172 15.5786 9.17157 14.8284C8.42143 14.0783 8 13.0609 8 12C8 10.9391 8.42143 9.92172 9.17157 9.17157C9.92172 8.42143 10.9391 8 12 8V6Z"
        fill="white"
      />
    </svg>
  );
}

/**
 * Nav label derived from the catalog title, dropping any parenthetical
 * suffix (e.g. "MVP (Most Valuable Person)" -> "MVP") so it fits the narrow
 * sticky column, matching the design's short nav labels.
 */
function navLabel(title: string): string {
  return title.split(" (")[0];
}

export function AwardCategoryNav() {
  const [activeSlug, setActiveSlug] = useState<string>(AWARDS_CATALOG[0]?.slug ?? "");

  useEffect(() => {
    if (typeof IntersectionObserver === "undefined") return;

    const sections = AWARDS_CATALOG.map((award) => document.getElementById(award.slug)).filter(
      (el): el is HTMLElement => el !== null,
    );
    if (sections.length === 0) return;

    const observer = new IntersectionObserver(
      (entries) => {
        const visible = entries.filter((entry) => entry.isIntersecting);
        if (visible.length === 0) return;
        const topMost = visible.reduce((a, b) => (a.boundingClientRect.top < b.boundingClientRect.top ? a : b));
        setActiveSlug(topMost.target.id);
      },
      { rootMargin: "-96px 0px -55% 0px", threshold: 0 },
    );

    sections.forEach((section) => observer.observe(section));
    return () => observer.disconnect();
  }, []);

  const handleClick = useCallback((event: React.MouseEvent<HTMLAnchorElement>, slug: string) => {
    event.preventDefault();
    setActiveSlug(slug);
    document.getElementById(slug)?.scrollIntoView({ behavior: "smooth", block: "start" });
  }, []);

  return (
    <nav
      aria-label="Award categories"
      className="sticky top-24 hidden w-44 flex-none flex-col gap-1 self-start lg:flex"
    >
      {AWARDS_CATALOG.map((award) => {
        const isActive = award.slug === activeSlug;
        return (
          <a
            key={award.slug}
            href={`#${award.slug}`}
            onClick={(event) => handleClick(event, award.slug)}
            aria-current={isActive ? "true" : undefined}
            className={`flex items-center gap-2 rounded px-4 py-4 text-sm font-bold transition-colors ${
              isActive
                ? "border-b border-[#FFEA9E] text-[#FFEA9E] [text-shadow:0_0_6px_#FAE287]"
                : "text-white hover:text-[#FFEA9E]"
            }`}
          >
            <TargetIcon className="h-6 w-6 shrink-0" />
            <span>{navLabel(award.title)}</span>
          </a>
        );
      })}
    </nav>
  );
}
